...
class DenseFeatureGP:
  ...
  def fit(self):
    ...
    ssqr = self.variance
    ll = 0.5*(z.T.dot(self.theta)/ssqr - logdet - self.y.T.dot(self.y)/ssqr - np.log(2*np.pi)*n - np.log(ssqr)*(n-dphi))
  ...
  def predict(self, X):
    ...
    var = self.variance * (1. + np.multiply(phiX, 
          scipy.linalg.cho_solve((self.factor, False), phiX.T).T).sum(axis=1, keepdims = True))
    return mu, var
